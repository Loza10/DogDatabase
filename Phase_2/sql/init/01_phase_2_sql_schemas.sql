...
CREATE USER IF NOT EXISTS shelterUser@'%' IDENTIFIED BY 'shelter123';
...
GRANT SELECT, INSERT, UPDATE, DELETE, FILE ON *.* TO 'shelterUser'@'%';
GRANT ALL PRIVILEGES ON `shelterUser`.* TO 'shelterUser'@'%';
GRANT ALL PRIVILEGES ON `cs6400_2025_01_Team025`.* TO 'shelterUser'@'%';
FLUSH PRIVILEGES;
...
CREATE TABLE Dog(
    ...
    PRIMARY KEY (dogID),
    CONSTRAINT phone_by_animal_control CHECK (by_animal_control = 0 OR surrender_phone IS NOT NULL AND surrender_phone != '')
);
